...
class LitWavesApp : public App {
public:
    LitWavesApp(HINSTANCE hInstanceHandle);
...
LitWavesApp::LitWavesApp(HINSTANCE hInstanceHandle) :
    App(hInstanceHandle)
{
}
...
void LitWavesApp::UpdateSunPosition(const GameTimer& gt)
{
...
void LitWavesApp::DrawRenderItems(const std::vector<RenderItem*>& ritems)
{
...
void LitWavesApp::Draw(const GameTimer& gt)
{
    ...
    auto passAddress = mCurrFrameResource->PassCB->Resource()->GetGPUVirtualAddress();
...
void LitWavesApp::BuildRootSignature()
{
    ...
    CD3DX12_ROOT_PARAMETER slotRootParamter[3];

    // Create root signature.
    slotRootParamter[0].InitAsConstantBufferView(0);    // Object CBV
    slotRootParamter[1].InitAsConstantBufferView(1);    // Material CBV
    slotRootParamter[2].InitAsConstantBufferView(2);    // Main Pass CBV
