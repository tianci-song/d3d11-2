...
void App::Update(const GameTimer& gt)
{
    ...
    mProj = dx::XMMatrixPerspectiveFovLH(mFov, AspectRatio(), 1.f, 1000.f);
...
void App::UpdateCamera()
{
    ...
    const float upComp = std::sin(mPitch + dx::XM_PIDIV2) >= 0 ? 1.f : -1.f;
    dx::XMVECTOR upDir = dx::XMVectorSet(0.f, upComp, 0.f, 0.f);
    ...
    dx::XMVECTOR lookAtDir = dx::XMVector4Transform(
        dx::XMVectorSet(0.f, 0.f, 1.f, 0.f),
        dx::XMMatrixRotationX(mPitch) * dx::XMMatrixRotationY(mYaw));
    
    // To calculate the lateral movement.
    dx::XMVECTOR lateralDir = dx::XMVector3Normalize(
        dx::XMVector3Cross(
            dx::XMVectorSet(0.f, 1.f, 0.f, 0.f),
            lookAtDir));
    
    // Save as XMFLOAT3, make the calculation easier.
    dx::XMStoreFloat3(&mLookAtDir, lookAtDir);
    dx::XMStoreFloat3(&mLateralDir, lateralDir);
    ...
    if (mIsOrbit)
    {
        mCameraPos = dx::XMVectorSubtract(mPivotPos, dx::XMVectorScale(lookAtDir, mRadius));
    }
    else
    {
        mPivotPos = dx::XMVectorAdd(mCameraPos, dx::XMVectorScale(lookAtDir, mRadius));
    }
    ...
    dx::XMMATRIX trans = dx::XMMatrixTranslationFromVector(dx::XMLoadFloat3(&mAxisOffset));

    mView = dx::XMMatrixLookAtLH(
        dx::XMVector4Transform(mCameraPos, trans),
        dx::XMVector4Transform(mPivotPos, trans),
        upDir);
}
...
{  
    const float dx = dx::XMConvertToRadians(static_cast<float>
        (x - mLastCursorPosOfWindow.x));
    const float dy = dx::XMConvertToRadians(static_cast<float>
...
void App::OnMouseScroll(WPARAM wParam, int x, int y)
{
    ...
    mFov = MathHelper::Clamp(
        mFov - sensitivity * dx::XMConvertToRadians(zDelta / WHEEL_DELTA),
        0.001f, 
        dx::XM_PI - 0.1f);
}
