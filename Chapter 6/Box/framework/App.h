...
protected:
	...
	wrl::ComPtr<ID3D12Resource> mSwapChainBuffer[SwapChainBufferCount];
	wrl::ComPtr<ID3D12Resource> mDepthStencilBuffer;
	
	wrl::ComPtr<IDXGIFactory4> mdxgiFactory;		// Create swap chain
	wrl::ComPtr<IDXGISwapChain> mSwapChain;
	wrl::ComPtr<ID3D12Device> md3dDevice;		// Create command queue, command allocator, command list, fence and get the size of descriptor

	wrl::ComPtr<ID3D12Fence> mFence;
	UINT64 mCurrentFence = 0;

	wrl::ComPtr<ID3D12CommandQueue> mCommandQueue;
	wrl::ComPtr<ID3D12CommandAllocator> mDirectCmdListAlloc;
	...
	wrl::ComPtr<ID3D12DescriptorHeap> mCbvHeap;
	...
	dx::XMMATRIX mWorldViewProj = dx::XMMatrixIdentity();
	...
	float mPitch = 0.f;
	bool  mIsOrbit = false;
	dx::XMVECTOR mCameraPos{ 0.f, 0.f, -mRadius, 1.f };
	dx::XMVECTOR mPivotPos{ 0.f, 0.f, 0.f, 1.f };
	dx::XMFLOAT3 mLookAtDir{ 0.f, 0.f, 1.f };
	dx::XMFLOAT3 mLateralDir{ 1.f, 0.f, 0.f };
	dx::XMFLOAT3 mAxisOffset{ 0.f, 0.f, 0.f };

	// Screen params
	float mFov = dx::XM_PIDIV4;
};