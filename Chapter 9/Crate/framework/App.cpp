...
void App::UpdateCamera()
{
    ...
    //
    // Move the camera along the lookat/lateral/world z-axis(not up dir) direction.
    //
    ...
    axisOffset.y = mCameraMoveSpeed * (
        ...
        //(mKeyMap['a'] + mKeyMap['d']) * mLateralDir.y +   // Lateral dir's y component is always 0, so omit it
...
{  
    ...
    float dx = XMConvertToRadians(static_cast<float>  // Not related with math, just shorten the value
        (x - mLastCursorPosOfWindow.x));
    float dy = XMConvertToRadians(static_cast<float>
    ...
    //
    // Update camera and pivot position that changed by mouse movement.
    //
    mCameraPos = XMVectorAdd(mCameraPos, XMLoadFloat3(&axisOffset));
    mPivotPos = XMVectorAdd(mPivotPos, XMLoadFloat3(&axisOffset));
