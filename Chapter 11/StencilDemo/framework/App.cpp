...
bool App::Initialize()
{
    if (!ResumeDataFromFile("saved.txt")) { return false; }

    if (!InitWindows()) { return false; }

    // Note: EnablePixGpuCapturer() must be called before InitDirect3D().
    // If you want to attach to an existing process and take a GPU Capture, 
    // then the process must have loaded a certain PIX DLL before calling D3D12 APIs 
    // such as D3D12CreateDevice or D3D12EnableExperimentalFeatures.
    if (!EnablePixGpuCapturer()) { return false; }
...
bool App::InitWindows()
{
    ...
    //
    // We need the client area to be 800x600, not the window area.(window includes client area and non-client area)
    // AdjustWindowRect is used to calculate the real window size based on the required 800x600 client area.
    //
    ...
    //
    // Get the screen size so we can know how to put the window in the center of screen.
    //
    RECT desktopRect;
    ...
    int posX = (desktopRect.right - width) / 2;
    int posY = (desktopRect.bottom - height) / 2;
...
bool App::EnablePixGpuCapturer()
{
    // Check to see if a copy of WinPixGpuCapturer.dll has already been injected into the application.
    // This may happen if the application is launched through the PIX UI. 
    if (GetModuleHandle(L"WinPixGpuCapturer.dll") == 0)
    {
        HMODULE result = LoadLibrary(GetLatestWinPixGpuCapturerPath().c_str());
        if (result == NULL)
        {
            return false;
        }
...
void App::UpdateCamera()
{
    ...
    //
    // Move the camera along the lookat/lateral/world-z direction.
    //
    XMFLOAT3 axisOffset{};
    axisOffset.x = mCameraMoveSpeed * (
        (mKeyMap['w'] + mKeyMap['s']) * mLookAtDir.x +
        (mKeyMap['a'] + mKeyMap['d']) * mLateralDir.x);
    axisOffset.y = mCameraMoveSpeed * (
        (mKeyMap['w'] + mKeyMap['s']) * mLookAtDir.y +
        //(mKeyMap['a'] + mKeyMap['d']) * mLateralDir.y +   // mLateralDir.y is always 0
        (mKeyMap['q'] + mKeyMap['e']) * 1.f);
    axisOffset.z = mCameraMoveSpeed * (
    ...
    mCameraPos = XMVectorAdd(mCameraPos, XMLoadFloat3(&axisOffset));
    mPivotPos = XMVectorAdd(mPivotPos, XMLoadFloat3(&axisOffset));

    mView = XMMatrixLookAtLH(mCameraPos, mPivotPos, upDir);
}
...
{  
    ...
    case MK_MBUTTON:
        axisOffset.x = dx * mLateralDir.x;
        axisOffset.y = -dy;
        axisOffset.z = dx * mLateralDir.z;
        break;
    case MK_LBUTTON:
        if (Input::IsKeyPressed(VK_MENU)) {
            mPitch += dy * mCameraRotSpeed;
            mYaw += dx * mCameraRotSpeed;
        }
        else {
            mYaw += dx * mCameraRotSpeed;
            axisOffset.x = -dy * mLookAtDir.x;
            axisOffset.z = -dy * mLookAtDir.z;
        }
        break;
    case MK_RBUTTON:
        if (Input::IsKeyPressed(VK_MENU)) {
            axisOffset.x = (-dy + dx) * mLookAtDir.x;
            axisOffset.y = (-dy + dx) * mLookAtDir.y;
            axisOffset.z = (-dy + dx) * mLookAtDir.z;
        }
        else {
            mPitch += dy * mCameraRotSpeed;
            mYaw += dx * mCameraRotSpeed;
...
bool App::ResumeDataFromFile(const char* filename)
{
    savedFile.open(filename, std::ios::in);
    if (!savedFile.is_open())
    {
        ...
        if (!file.is_open())
        {
            MessageBoxA(NULL, "Unable to open file!", NULL, MB_OK);
            return false;
        }
    ...
    XMFLOAT3 cameraPos{};
    savedFile.ignore(MAX_SIZE, '{');
    savedFile.ignore(1);    // ignore 1 space
    for (int i = 0; i < 3; ++i)
    {
        savedFile >> ((float*)&cameraPos)[i];
        savedFile.ignore(2);
    }

    mCameraPos = XMLoadFloat3(&cameraPos);
...
bool App::SaveDataBeforeExit(const char* filename)
{
    savedFile.open(filename, std::ios::out | std::ios::trunc);
    if (!savedFile.is_open())
    {
        MessageBoxA(NULL, "Unable to open file!", NULL, MB_OK);
        return false;
