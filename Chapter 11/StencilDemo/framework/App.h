...
protected:
	...
	bool ResumeDataFromFile(const char* filename);
	bool SaveDataBeforeExit(const char* filename);
...
protected:
	...
	float mPitch = 0.f;
	bool  mIsOrbit = false;
	float mCameraMoveSpeed = 0.004f;
	float mCameraRotSpeed = 0.2f;
	XMVECTOR mCameraPos{ 0.f, 0.f, 0.f, 1.f };
	...
	// Filestream used to save the data in when program exits.
