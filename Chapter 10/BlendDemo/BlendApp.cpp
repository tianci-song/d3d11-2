...
class BlendApp : public App {
public:
    BlendApp(HINSTANCE hInstanceHandle);
...
BlendApp::BlendApp(HINSTANCE hInstanceHandle) :
    App(hInstanceHandle)
{
}
...
void BlendApp::Draw(const GameTimer& gt)
{
    ...
    //
    // Then rendering transparent objects.
    //
...
void BlendApp::LoadTexture()
{
    ...
    CreateDDSTextureFromFile12(
        md3dDevice.Get(),
        mCommandList.Get(),
        wireFenceTex->Filename.c_str(),
        wireFenceTex->Resource,
        wireFenceTex->UploadHeap) >> chk;
...
void BlendApp::BuildDescriptorHeaps()
{
    ...
    const auto& wireFenceTex = mTextures["wireFenceTex"]->Resource;
...
void BlendApp::UpdateSunPosition(const GameTimer& gt)
{
...
void BlendApp::DrawRenderItems(const std::vector<RenderItem*>& ritems)
{
...
std::array<const D3D12_STATIC_SAMPLER_DESC, 6> BlendApp::GetStaticSamplers()
{
