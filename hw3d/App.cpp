...
bool App::Initialize(HINSTANCE instanceHandle, int show)
{
    if (!InitWindows(instanceHandle, show)) { return false; }
    ...
    md3dDevice->CreateCommittedResource(
        ...
        &optClear,
        IID_PPV_ARGS(mDepthStencilBuffer.GetAddressOf())) >> chk;
...
/* Version A: a basic message loop */
/*
int App::Run()
{
    MSG msg = { 0 };

    // This function will loop until get the WM_QUIT message.
    // Once receive the WM_QUIT, GetMessage() returns 0,
    // once error occured, it will return -1.
    // If no message received, it will turn into sleeping state.
    BOOL bRet = 1;
...
/* Version B: a more flexible message loop */
/* GetMessage() will wait until it get new message, but wasting time,
   not fit in game development. The improvement is using PeekMessage(),
   returns 0 when no message received, so that we can do other job. */
int App::Run()
{
    mTimer.Reset();
...
bool App::InitWindows(HINSTANCE instanceHandle, int show)
{
    WNDCLASS wc = {};
    ...
    wc.hIcon = (HICON)LoadImage(instanceHandle, MAKEINTRESOURCE(IDI_ICON1), IMAGE_ICON, 32, 32, LR_DEFAULTCOLOR);
    ...
    mhMainWnd = CreateWindow(
        ...
        0,  // Parent window
        0,  // Menu handle
        instanceHandle, // App instance handle
...
void App::CreateCommandObjects()
{
    ...
    md3dDevice->CreateCommandAllocator(
        ...
        IID_PPV_ARGS(mDirectCmdListAlloc.GetAddressOf())) >> chk;
    ...
    md3dDevice->CreateCommandList(
        ...
        IID_PPV_ARGS(mCommandList.GetAddressOf())) >> chk;
...
void App::CreateRtvAndDsvDescriptorHeaps()
{
    ...
    md3dDevice->CreateDescriptorHeap(
        &rtvHeapDesc, IID_PPV_ARGS(mRtvHeap.GetAddressOf())) >> chk;
    ...
    md3dDevice->CreateDescriptorHeap(
        &dsvHeapDesc, IID_PPV_ARGS(mDsvHeap.GetAddressOf())) >> chk;
}
